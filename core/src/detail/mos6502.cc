...
void MOS6502::FetchOpcode() {
  ...
  switch (GetAddressingMode()) {
  case AddressingMode::ABS: Stage([this] { SetAddressLo(Read(REGISTER(pc)++));                                  });
                            Stage([this] { SetAddressHi(Read(REGISTER(pc)++));                                  });
                            Stage([this] { Fetch();                        }, Is(MemoryAccess::READ_MODIFY_WRITE));
                            Stage([this] { Write(GetAddress(), Fetched()); }, Is(MemoryAccess::READ_MODIFY_WRITE)); break;
  case AddressingMode::ZP0: Stage([this] { SetAddress(Read(REGISTER(pc)++));                                    });
                            Stage([this] { Fetch();                        }, Is(MemoryAccess::READ_MODIFY_WRITE));
                            Stage([this] { Write(GetAddress(), Fetched()); }, Is(MemoryAccess::READ_MODIFY_WRITE)); break;
  ...
  case Instruction::ASL: Stage([this] { if (Is(AddressingMode::ACC)) REGISTER(a) = ShiftL(REGISTER(a), false);
                                        else Write(GetAddress(), ShiftL(Fetched(), false));                    }); break;
  ...
  case Instruction::JMP: /* Defined above. */                                                                      break;
  case Instruction::JSR: /* Defined above. */                                                                      break;
  ...
  case Instruction::LSR: Stage([this] { if (Is(AddressingMode::ACC)) REGISTER(a) = ShiftR(REGISTER(a), false);
                                        else Write(GetAddress(), ShiftR(Fetched(), false));                    }); break;
  ...
  case Instruction::PHP: Stage([this] { Read(REGISTER(pc));                                                    });
                         Stage([this] { Push(REGISTER(p) | MASK(brk_command) | MASK(unused));
                                        REGISTER(p) &= ~(MASK(brk_command) | MASK(unused));                    }); break;
  ...
  case Instruction::PLP: Stage([this] { Read(REGISTER(pc));                                                    });
                         ...
                         Stage([this] { REGISTER(p) = Pull();
                                        REGISTER(p) |= MASK(unused);                                           }); break;
  case Instruction::ROL: Stage([this] { if (Is(AddressingMode::ACC)) REGISTER(a) = ShiftL(REGISTER(a), true);
                                        else Write(GetAddress(), ShiftL(Fetched(), true));                     }); break;
  case Instruction::ROR: Stage([this] { if (Is(AddressingMode::ACC)) REGISTER(a) = ShiftR(REGISTER(a), true);
                                        else Write(GetAddress(), ShiftR(Fetched(), true));                     }); break;
